 ...
 * Behaviour is preserved; only the language surface was modernised:
 ...
 *     (the manual OpenLibrary calls and global bases are kept as-is)
 *
 * Unlike the Ghosts harness, this one draws with Intuition Border (vector)
 * objects via DrawBorder(), not DrawImage(). The blitter is not fed source
 * data from our arrays, so there is NO Chip-RAM requirement here — the maze
 * coordinate tables can live anywhere.
 */
...
/* Demonstrates AutoRequest(); kept from the original harness (not called by
 * main). Left as a non-static global so -Wall does not flag it unused. */
void about(void)
{
	AutoRequest(wPacMan,
		&my_body_text, 
...
int main(void)
{
	...
	/* The original held the maze for Delay(600) (~10s at 60Hz) then exited.
	 * The headless emu workflow screenshots just before EMU_TIMEOUT, so hold
	 * the maze on screen well past that; it still exits cleanly on real HW. */
	Delay(3000);
